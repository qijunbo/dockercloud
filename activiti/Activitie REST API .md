...
说明
----
  本方案主要考虑的是Activiti 微服务系统边界的划分和系统接口的定义, 暂不确定具体实现.  至于实现方式是沿用现有的代码还是使用官方定义的接口,各有利弊.
  
  我们现有的系统提供了基于事件的邮件发送, 和流程的驳回等定制化功能, 如果采用官方的接口,这些功能应该加上去.
  
  如果采用现有的代码,  那么应该修改接口的形式. 因为现有的代码多数不是以微服务的方式对外提供接口的, 而是以service的方式供直接调用.   少数的几个Controller 提供的API不符合RESTful API的命名规范, 需要统一规范.
